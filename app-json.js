...
class JSONDatabase {
    ...
    initializeEventListeners() {
        ...
        // File operations
        document.getElementById('loadDataBtn').addEventListener('click', () => this.loadSpecificFile('data.json'));
    ...
    handleFileSelect(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                this.data = this.parseJSON(jsonText);
    ...
    saveToFile() {
        this.currentFile = 'data.json';
        const jsonText = this.stringifyJSON(this.data);
    ...
    downloadFile() {
        const jsonText = this.stringifyJSON(this.data);
        const fileName = this.currentFile || 'database.json';
        const blob = new Blob([jsonText], { type: 'application/json;charset=utf-8;' });
    ...
    loadDataFile() {
        // Automatically try to load data.json using fetch
        this.showStatus('Loading data.json...');
        ...
        fetch('data.json')
            ...
            .then(data => {
                ...
                this.updateFileInfo();
                this.updateDisplay();
                this.showStatus('Loaded data.json with ' + this.data.length + ' records');
            })
            .catch(error => {
                // If data.json doesn't exist, load sample data
    ...
    loadSampleDataFallback() {
        ...
        this.currentFile = 'data.json';
        this.updateDisplay();
        this.showStatus('Sample data loaded. To load data.json, use the File Management section.');
    }
